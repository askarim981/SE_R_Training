...
The advanced technicalities of UDFs are, in most instances, outstide the scope of this book. If you would like to read more about the specifics covered here, some good resources are:
...
While loops are useful when we don't know exactly when to terminate the loop. They do, however, require us to define an evolving condition. More common is to know the set around which we need to loop. This will usually be a number or vector (remember vectors can vary in length). 
