...
```{r eval=FALSE}
---
title: "Outputs for Skills Needs Assessments"
author: "Abdus-Subhaan Karim"
date: "`r Sys.Date()`"
output: 
  html_document:
    theme: flatly
...
## {.tabset .tabset-pills}
### Baseline {tabset}
#### Occupation explorer graph
ggplotly(baseline_graph, tooltip = "text")

### Scenario 1 - Technological Acceleration {.tabset}
#### Occupation explorer graph
ggplotly(scenario1_graph, tooltip = "text")

### Scenario  - Human-centricity {.tabset}
#### Occupation explorer graph
ggplotly(scenario2_graph, tooltip = "text")

```

```{r, echo=FALSE, out.width='80%', fig.align='center'}
knitr::include_graphics('images and graphics/Markdown_tabs_SNAs.jpg')
```
...
```{r Outputs quiz, echo=FALSE, results='asis'}
...
create_outputs_quiz <- function() {
  ...
  # Loop through to print questions as a numbered list
  for (i in seq_along(questions)) {
    # Start each question with markdown for a numbered list item
    cat(i, ". **", questions[[i]]$q, "**\n\n", sep = "")
    # Print options, each on a new line
    for (option in questions[[i]]$options) {
      cat("    ", option, "\n") # Indent options for clarity
    }
    cat("\n") # Add a blank line after each question's options
  }
  ...
  # Loop through to print answers as a numbered list
  for (i in seq_along(questions)) {
